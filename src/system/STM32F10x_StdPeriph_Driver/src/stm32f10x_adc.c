...
void ADC_StructInit(ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  ADC_InitStruct->ADC_NbrOfChannel = 1U;
}
...
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  if (ADC_Channel > ADC_Channel_9)
  {
    ...
    {
      /*
        ORIG tmpreg2 = (((uint32_t)SMPR1_SMP_Set) << (3U * (((uint32_t)(ADC_Channel)) - 10U)));
       */
      uint32_t u32_tmp = SMPR1_SMP_Set;
      tmpreg2 = u32_tmp << ((uint32_t)(3U * ((uint32_t)ADC_Channel - 10U)));
    }
    /* Clear the old channel sample time */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_SampleTime << (3U * (ADC_Channel - 10U));
  ...
  else /* ADC_Channel include in ADC_Channel_[0..9] */
  {
    ...
    {
      /* ORIGIN
        tmpreg2 = SMPR2_SMP_Set << (3U * ADC_Channel);
      */
      uint32_t u32_tmp = SMPR2_SMP_Set;
      tmpreg2 = u32_tmp << (3U * ADC_Channel);
    }
    /* Clear the old channel sample time */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_SampleTime << (3U * ADC_Channel);
    /* Set the new channel sample time */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SMPR2 = tmpreg1;
  ...
  if (Rank < 7U)
  {
    ...
    tmpreg1 = ADCx->SQR3;
    /* Calculate the mask to clear */
    /* ORIGIN tmpreg2 = SQR3_SQ_Set << (5U * (Rank - 1U)); */
    {
      uint32_t u32_tmp = SQR3_SQ_Set;
      tmpreg2 = u32_tmp << (5U * (Rank - 1U));
    }

    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_Channel << (5U * (Rank - 1U));
  ...
  else if (Rank < 13U)
  {
    ...
    /* Calculate the mask to clear */
    {
      uint32_t u32_tmp = SQR2_SQ_Set;
    tmpreg2 = u32_tmp << (5U * (Rank - 7U));
    }
    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_Channel << (5U * (Rank - 7U));
    /* Set the SQx bits for the selected rank */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SQR2 = tmpreg1;
  ...
  else
  {
    ...
    tmpreg1 = ADCx->SQR1;
    /* Calculate the mask to clear */
    { uint32_t u32_tmp = SQR1_SQ_Set;
      tmpreg2 = u32_tmp << (5U * (Rank - 13U));
    }
    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_Channel << (5U * (Rank - 13U));
    /* Set the SQx bits for the selected rank */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SQR1 = tmpreg1;
...
{
  uint32_t tmpreg1 = 0U, tmpreg2 = 0U, tmpreg3 = 0U;
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
  assert_param(IS_ADC_CHANNEL(ADC_Channel));
  ...
  if (ADC_Channel > ADC_Channel_9)
  {
    ...
    {
      uint32_t u32_tmp = SMPR1_SMP_Set;
      tmpreg2 = u32_tmp << (3U * (ADC_Channel - 10U));
    }
    /* Clear the old channel sample time */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
  ...
  {
    uint32_t u32_tmp = JSQR_JSQ_Set;
    tmpreg2 = u32_tmp << (5U * (uint8_t)( (Rank + 3U) - (uint8_t)(tmpreg3 + 1U) ));
  }
  /* Clear the old JSQx bits for the selected rank */
  tmpreg1 &= ~tmpreg2;
  /* Calculate the mask to set: ((Rank-1)+(4-JL-1)) */
  tmpreg2 = (uint32_t)ADC_Channel << (5U * (uint8_t)((Rank + 3U) - (uint8_t)(tmpreg3 + 1U)));
...
ITStatus ADC_GetITStatus(ADC_TypeDef* ADCx, uint16_t ADC_IT)
{
  ...
  uint32_t itmask = 0U, enablestatus = 0U;
  /* Check the parameters */
  assert_param(IS_ADC_ALL_PERIPH(ADCx));
